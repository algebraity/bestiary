...
static constexpr Section kSections[] = {
    {
        ...
        "* Graphing: Use the `\\graph{function}` or `\\graph{expression}` command to graph a function in a new Graph tab, or `\\graph{...}{n}` to add a function to the nth Graph tab\n\n"
        "* 3D Graphing: Use the `\\graph3D{function}` or `\\graph3D{expression}{n} command to create a 3D graph. Works the same was as `\\graph`, and supports z = f(x, y) and z = f(t, x)."
        ...
        "* Loops: Use for and while loops via the commands `\\for{i = start; i < end; inc (e.g. i += 1)}{stuff}` and `\\while{condition}{stuff}`.\n\n"
        "* Conditionals: Invoke conditionals via the command `\\if{condition}{stuff}{else this stuff}`.\n\n"
...
static constexpr Command kCommands[] = {
    ...
    HELP_CMD("solveQuadratic", -1, Beast::Hebi, "Solves a quadratic equation by the exact quadratic formula over real or complex coefficients.", "polynomial in x, List of coefficients, or coefficients a,b,c for a*x^2+b*x+c", "List of exact formula expressions"),
    HELP_CMD("solveCubic", -1, Beast::Hebi, "Solves a cubic equation by the exact cubic formula over real or complex coefficients.", "polynomial in x, List of coefficients, or coefficients a,b,c,d for a*x^3+b*x^2+c*x+d", "List of exact formula expressions"),
    HELP_CMD("solveQuartic", -1, Beast::Hebi, "Solves a quartic equation by the exact quartic formula over real or complex coefficients.", "polynomial in x, List of coefficients, or coefficients a,b,c,d,e for a*x^4+b*x^3+c*x^2+d*x+e", "List of exact formula expressions"),
    ...
    HELP_CMD("AP", -1, Beast::Ookami, "Constructs an arithmetic progression as a finite integer set.", "Int first term, Int common difference, Int number of terms >= 0", "CombSet"),
    HELP_CMD("GP", -1, Beast::Ookami, "Constructs a geometric progression as a finite integer set.", "Int first term, Int common ratio, Int number of terms >= 0", "CombSet"),
